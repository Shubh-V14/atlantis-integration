...
resource "aws_s3_bucket_lifecycle_configuration" "expire" {
  count = var.expiry["enabled"] ? 1: 0
  rule {
    id      = "expire"
    status  = "Enabled"

    expiration {
      days = var.expiry["days"]
    }
  }
...
resource "aws_iam_policy" "upswing_bucket_policy" {
    description = "${local.bucket_name}-bucket-permissions"
    name        = "${local.bucket_name}-bucket-permissions"
    policy      = <<EOT
{
    "Version": "2012-10-17",
    "Statement": [
