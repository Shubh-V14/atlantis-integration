# aws_iam_policy.backend_frame_policy:
resource "aws_iam_policy" "backend_frame_policy" {
    description = "backend-frame-permissions"
    name        = "backend-frame-permissions"
    policy      = <<EOT
{
    "Version": "2012-10-17",
    "Statement": [
...
resource "aws_iam_role_policy_attachment" "attach_frame_permissions_to_service_role" {
  ...
  policy_arn = aws_iam_policy.backend_frame_policy.arn
}
