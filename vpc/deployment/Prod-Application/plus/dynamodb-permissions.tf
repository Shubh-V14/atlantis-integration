locals {
    services_for_dynamodb_permissions = ["payments-service", "pam-service"]
    namespace = "in1"
}

# aws_iam_policy.ups_prod_customer_service_policy:
resource "aws_iam_policy" "ups_prod_dynamodb_policy" {
    count = length(local.services_for_dynamodb_permissions)
    description = "ups-prod-${local.services_for_dynamodb_permissions[count.index]}-permissions"
    name = "ups-prod-${local.services_for_dynamodb_permissions[count.index]}-permissions"
    policy      = <<EOT
{
    "Version": "2012-10-17",
    "Statement": [
...
resource "aws_iam_role_policy_attachment" "attach_dynamodb_permissions_to_service_role" {
  ...
  policy_arn = aws_iam_policy.ups_prod_dynamodb_policy[count.index].arn
}