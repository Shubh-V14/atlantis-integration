locals {
    customer_support_attachments_bucket_name = "customer-support-attachments-prod"
}
...
resource "aws_iam_role_policy_attachment" "attach_tratod_permissions_to_customer_support_service_role" {
  count = length(local.service_env_set_for_customer_support_permissions)
  role       = data.aws_iam_role.customer_support_service_role[count.index].name
  policy_arn = module.s3_trat_od.iam_policy_arn
}