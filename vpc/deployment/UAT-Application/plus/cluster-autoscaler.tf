    ...
    "Statement": [
        {
            ...
            "Condition": {
                "StringEquals": {
                    "aws:ResourceTag/k8s.io/cluster-autoscaler/uat-application1": "owned"
                }
            }
        },
        {
...
EOT
    ...
    tags = merge(local.tags,tomap({"Name" = "cluster-autoscaler-permissions"}))
}
...
EOT
    ...
    path                  = "/"
    tags = merge(local.tags,tomap({"Name" = "cluster-autoscaler-role"}))
