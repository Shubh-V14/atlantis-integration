...
resource "aws_kms_key" "cde_alpha_tokenisation_key" {
...
output "cde_alpha_tokenisation_key_arn" {
  value = aws_kms_key.cde_alpha_tokenisation_key.arn
}

resource "aws_kms_alias" "cde_alpha_tokenisation_key_alias" {
  name          = "alias/stage_cde_alpha_tokenisation"
  target_key_id = aws_kms_key.cde_alpha_tokenisation_key.id
}

# aws_iam_policy.ups_stage_cde_alpha_tokenisation_policy:
resource "aws_iam_policy" "ups_stage_cde_alpha_tokenisation_policy" {
    description = "ups-stage-cde-alpha-tokenisation-permissions"
    name        = "ups-stage-cde-alpha-tokenisation-permissions"
    policy      = <<EOT
{
    "Version": "2012-10-17",
    "Statement": [
   ...
   "Resource": "${aws_kms_key.cde_alpha_tokenisation_key.arn}"
}
    ]
}
EOT
...
resource "aws_iam_role_policy_attachment" "attach_cde_tokenisation_permissions_to_service_role" {
  ...
  policy_arn = aws_iam_policy.ups_stage_cde_alpha_tokenisation_policy.arn
}

resource "aws_iam_role_policy_attachment" "attach_bank_keys_permissions_to_service_role" {
  count = length(local.service_env_set_for_cde_tokenisation_permissions)
  role       = data.aws_iam_role.cde_tokenisation_service_role[count.index].name
  policy_arn = aws_iam_policy.access_to_alpha_bank_keys.arn
}