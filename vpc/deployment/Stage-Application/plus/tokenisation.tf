...
resource "aws_kms_key" "alpha_tokenisation_key" {
...
output "alpha_tokenisation_key_arn" {
  value = aws_kms_key.alpha_tokenisation_key.arn
}

resource "aws_kms_alias" "alpha_tokenisation_key_alias" {
  name          = "alias/stage_tokenisation"
  target_key_id = aws_kms_key.alpha_tokenisation_key.id
}

# aws_iam_policy.ups_stage_tokenisation_policy:
resource "aws_iam_policy" "ups_stage_tokenisation_policy" {
    description = "ups-stage-tokenisation-permissions"
    name        = "ups-stage-tokenisation-permissions"
    policy      = <<EOT
{
    "Version": "2012-10-17",
    "Statement": [
   ...
   "Resource": "${aws_kms_key.alpha_tokenisation_key.arn}"
}
    ]
}
EOT
...
resource "aws_iam_policy" "access_to_alpha_bank_keys" {
  ...
  policy                   = jsonencode(
            ...
            Statement = [
      {
       ...
       "kms:CreateGrant" 
        ...
        "Resource"= aws_kms_key.alpha_bank_keys[*].arn
        }
            ]
            Version   = "2012-10-17"
        }
