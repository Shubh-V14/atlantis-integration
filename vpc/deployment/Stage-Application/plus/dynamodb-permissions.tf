...
# aws_iam_policy.ups_stage_customer_service_policy:
...
resource "aws_iam_policy" "ups_stage_dynamodb_policy" {
    ...
    description = "ups-stage-${local.service_env_set_for_dynamodb_permissions[count.index][0][0]}-${local.service_env_set_for_dynamodb_permissions[count.index][1]}-permissions"
    name = "ups-stage-${local.service_env_set_for_dynamodb_permissions[count.index][0][0]}-${local.service_env_set_for_dynamodb_permissions[count.index][1]}-permissions"
    policy      = <<EOT
{
    "Version": "2012-10-17",
    "Statement": [
